...
def create_alerts(config, entries):
    ...
    for alert in entries:
        try:
            service.saved_searches[alert.name]
            savedsearch = gen_savedsearch_alert_param(alert.content)
            mysavedsearch.update(**savedsearch).refresh()
            print "Updated: " + mysavedsearch.name
        except:
            try:
                mysavedsearch = service.saved_searches.create(alert.name, alert.content.search.replace('test', 'tpa'))
                savedsearch = gen_savedsearch_alert_param(alert.content)
                mysavedsearch.update(**savedsearch).refresh()
